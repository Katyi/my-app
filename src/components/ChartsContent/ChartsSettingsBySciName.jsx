...
import { GET_CHARTS_SUGGESTIONS_LAT_SCIENTIFIC_NAME } from "../../apollo/getSuggestions";
...
function ChartsSettingsBySciName({ onNameChanged, initial = "", color_list, onColorChanged=(plant,color)=>{} }) {
  ...
  const useSuggestionLat = () => {
    const usageLatScientificName = useLazyQuery(
      GET_CHARTS_SUGGESTIONS_LAT_SCIENTIFIC_NAME
    );
      ...
      ? [
          ...new Map(
            suggestionLat[0][1]?.data?.usages.map((option) => {
              return [option.scientificName[0].name+'$$$'+ option.scientificName[0].rusNomenclatureName , option];
            })
          ).keys(),
        ]
      : [];
                            ...
                            <Stack key={itemInd + "   "} direction={"row"} alignItems={"center"}>
                              ...
                              >
                                ...
                                <Autocomplete
                                  ...
                                  onChange={(e, value) => {
                                    if (value) {
                                      setFieldValue(`list[${itemInd}]`, value?.split("$$$").join(" - "));
                                      onNameChanged(
                                        values.list.map((el, i) =>
                                          i === itemInd ? value?.split("$$$").join(" - ") : el
                                        )
                                      );
                                    } else {
